...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('add')
    .setDescription('Add a user or role to a ticket channel. | Añade un usuario o rol al ticket.')
    .addUserOption((option) =>
      option.setName('user').setDescription('Select a user | Selecciona un usuario').setRequired(false),
    )
    .addRoleOption((option) =>
      option.setName('role').setDescription('Select a role | Selecciona un rol').setRequired(false),
    )
  ...
  async execute(interaction) {
    ...
    if (user) {
      ...
      if (interaction.channel.members.has(user.id)) {
        return interaction.reply({
          content: 'That user is already in this ticket. | Este usuario ya esta en el ticket.',
          ephemeral: true,
        });
      }
    ...
    if (role) {
      ...
      if (interaction.channel.permissionsFor(role.id).has('ViewChannel')) {
        return interaction.reply({
          content: 'That role is already in this ticket. | Este rol ya esta en el ticket.',
          ephemeral: true,
        });
      }
