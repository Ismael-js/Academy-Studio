...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('unclaim')
    .setDescription('Unclaim a ticket. | Desclamar un ticket.')
  ...
  async execute(interaction) {
    ...
    if (config.claimFeature === false) {
      return interaction.reply({
        content: 'The claim feature is currently disabled. | La función de reclamación está actualmente desactivada.',
        ephemeral: true,
      });
    }
    ...
    if ((await ticketsDB.get(`${interaction.channel.id}.claimed`)) === false) {
      return interaction.reply({
        content: 'This ticket has not been claimed! | Este ticket no ha sido reclamado.',
        ephemeral: true,
      });
    }
    ...
    if (
      ...
      return interaction.reply({
        content: `You did not claim this ticket, only the user that claimed this ticket can unclaim it! (<@!${await ticketsDB.get(`${interaction.channel.id}.claimUser`)}>) | Usted no reclamó este billete, sólo el usuario que lo reclamó puede anular la reclamación. (<@!${await ticketsDB.get(`${interaction.channel.id}.claimUser`)}>)`,
        ephemeral: true,
      });
    }
    ...
    interaction.editReply({
      content: 'You successfully unclaimed this ticket! | Ha sacado el reclamo correctamente del ticket',
      ephemeral: true,
    });
