    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.customId === 'deleteTicket') {
        ...
        const logEmbed = new EmbedBuilder()
          ...
          .addFields([
            {
              name: '<:soporte:1146961138211373088> Deleted By',
              value: `> <@!${interaction.user.id}>\n> ${sanitizeInput(interaction.user.tag)}`,
            },
            {
              name: '<:member:1144564282403598396> Ticket Creator',
            ...
            {
              name: '<:idhastag:1144565372041822238> Ticket',
              value: `> #${sanitizeInput(interaction.channel.name)}\n> ${await ticketsDB.get(`${interaction.channel.id}.ticketType`)}`,
            },
          ])
          .setTimestamp()
        ...
        if (config.DMUserSettings.enabled) {
          const dmEmbed = new EmbedBuilder()
            ...
            .addFields(
              {
                name: '<:verifyorg:1147167104508903424> Server',
                value: `> ${interaction.guild.name}`,
                inline: true,
              },
              {
                name: '<:newticket:1146966575396700281> Ticket',
                value: `> #${sanitizeInput(interaction.channel.name)}`,
                inline: true,
              },
              {
            ...
            .addFields(
              {
                name: '<:member:1144564282403598396> Ticket Author',
              ...
              {
                name: '<:report:1146961135745126420> Claimed By (Optional)',
                value: `> ${claimUser ? sanitizeInput(claimUser.tag) : 'None'}`,
                inline: true,
              },
              {
                name: '<:greendiamond:1144537365549957230> Terms and Conditions',
                value: `> Accepted`,
                inline: true,
              },
            );
          await ticketUserID.send({ embeds: [dmEmbed] });
      ...
      if (interaction.customId === 'closeTicket') {
        ...
        const logEmbed = new EmbedBuilder()
          ...
          .addFields([
            {
              name: '<:arrowred:1146968528172368014> Closed By',
              value: `> <@!${interaction.user.id}>`,
            },
            {
              name: '<:arrowred:1146968528172368014> Ticket Creator',
              value: `> <@!${ticketUserID.id}>`,
            },
            {
              name: '<:arrowred:1146968528172368014> Ticket',
              value: `> #${sanitizeInput(interaction.channel.name)}\n> ${await ticketsDB.get(`${interaction.channel.id}.ticketType`)}`,
            },
          ])
          .setTimestamp()
        ...
        if (claimUser)
          logEmbed.addFields({
            name: '<:stafficon:1146963324509102170> Claimed By',
            value: `> <@!${claimUser.id}>`,
          });
        let logsChannel = interaction.guild.channels.cache.get(
          config.logs_channel_id,
        ...
        interaction.channel.messages
          ...
          .then(async (message) => {
            ...
            const logEmbed = new EmbedBuilder()
              ...
              .addFields([
                {
                  name: '<:stafficon:1146963324509102170> Staff',
                  value: `> <@!${interaction.user.id}>`,
                },
                {
                  name: ':idhastag:1144565372041822238> Ticket',
                ...
                await channel
                  ...
                  .then(async (message) => {
                    let newTicketOpened = new EmbedBuilder()
                      ...
                      .setColor(config.default_embed_color)
                      .setDescription(
                        `Your new ticket (<#${channel.id}>) has been created, **${sanitizeInput(interaction.user.username)}!**\n<:whitesmall:849113182177984572> <:eeuu:1205298027796103188> | Before going to the ticket, please read the terms and conditions.\n<:Alert:1205544581727526992> **Alert**: If you did not read our terms and conditions and join the ticket then you accept the Academy Studios terms and conditions.\n\n<:whitesmall:849113182177984572> <:spain:1205297979997687828> | Antes de acceder al ticket, lea las condiciones.\n<:Alert:1205544581727526992> **Alerta**: Si no has leído nuestros términos y condiciones y te unes al ticket, entonces aceptas los términos y condiciones de Academy Studios.`,
                      )
                      .setFooter({
                        text: `${interaction.user.tag}`,
                        iconURL: `${interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 })}`,
                      ...
                      .setTimestamp();
                    const btnTerm = new ButtonBuilder()
                        .setStyle(ButtonStyle.Link)
                        .setURL(`${channel.url}`)
                        .setLabel('Terms & Conditions')
                        .setEmoji('📝');

                    const btnAccepted = new ButtonBuilder()
                    .setCustomId('btnaccepted')
                    .setStyle(ButtonStyle.Success)
                    .setLabel('Accepted')
                    .setEmoji('💚');

                    const actionRow4 = new ActionRowBuilder().addComponents(btnTerm, btnAccepted);
                    ...
                    const logEmbed = new EmbedBuilder()
                      ...
                      .addFields([
                        {
                          name: '<:arrow_orange:1205113834965041193> Ticket Creator',
                          value: `> <@!${interaction.user.id}>`,
                        },
                        {
                          name: '<:arrow_orange:1205113834965041193> Ticket',
